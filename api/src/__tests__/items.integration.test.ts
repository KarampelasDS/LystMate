...
const mockItemFindUnique = prisma.item.findUnique as jest.Mock;
const mockItemFindMany = prisma.item.findMany as jest.Mock;
const mockItemCreate = prisma.item.create as jest.Mock;
const mockItemUpdate = prisma.item.update as jest.Mock;
const mockItemDelete = prisma.item.delete as jest.Mock;
const mockItemCount = prisma.item.count as jest.Mock;
...
describe("Items API", () => {
  ...
  describe("POST /lists/:listId/items", () => {
    ...
    it("returns 401 when no token provided", async () => {
      const res = await request(app).post("/lists/list-1/items").send({ name: "Milk" });
  ...
  describe("GET /lists/:listId/items", () => {
    ...
    it("caps limit at 100", async () => {
      ...
      const res = await request(app).get("/lists/list-1/items?limit=999").set(AUTH);
      expect(res.status).toBe(200);
